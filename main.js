...
document.addEventListener('click',(event)=>{
    ...
    if(edit_status == Boolean(true)){
        ...
        if(event.target.classList.contains('yoko')){
            piece_override()
            if(select_status == "hu" && piece_hu_stok > 0){
                event.target.classList.add('piece-hu');
                piece_hu_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "king" && piece_king_stok > 0){
                    event.target.classList.add('piece-king');
                    piece_king_stok--;
                    sound_effect.currentTime = 0;
                    sound_effect.play();
            }
            else if(select_status == "kin" && piece_kin_stok > 0){
                event.target.classList.add('piece-kin');
                piece_kin_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "gin" && piece_gin_stok > 0){
                event.target.classList.add('piece-gin');
                piece_gin_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "kei" && piece_kei_stok > 0){
                event.target.classList.add('piece-kei');
                piece_kei_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "kyou" && piece_kyou_stok > 0){
                event.target.classList.add('piece-kyou');
                piece_kyou_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "hi" && piece_hi_stok > 0){
                event.target.classList.add('piece-hi');
                piece_hi_stok--;
                sound_effect.currentTime = 0;
                sound_effect.play();
            }
            else if(select_status == "kaku" && piece_kaku_stok > 0){
